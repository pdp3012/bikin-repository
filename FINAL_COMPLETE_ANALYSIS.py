...
# SERTIFIKASI INTERNASIONAL DALAM DATA SCIENCE DAN MACHINE LEARNING
# 
# DESKRIPSI:
# Kode ini merupakan implementasi lengkap analisis kompetitor dan pemetaan 
# spasial untuk data produk sarung tangan dari Tokopedia. Kode mencakup:
# 1. Visualisasi choropleth map yang sophisticated dengan legenda informatif
# 2. Analisis cluster dengan HDBSCAN
# 3. Heatmap density dengan analisis spasial
# 4. Visualisasi interaktif dengan Plotly
# 5. Analisis kompetitor dan rekomendasi strategi
# 
# FITUR UTAMA:
# - Choropleth map dengan gradasi warna lightgreen → yellow → orange → red
# - Legend yang sophisticated dengan informasi detail
# - Popup interaktif dengan statistik per wilayah
# - Heatmap density berdasarkan cluster analysis
# - 3D scatter plot dan parallel coordinates
...
# =============================================================================
# LANGKAH 1: GENERATOR DATA DUMMY UNTUK DEMONSTRASI
# =============================================================================

def generate_dummy_data(n_samples=1000):
    """
    Generate data dummy untuk demonstrasi analisis spasial
    Menggunakan teknik sampling yang sophisticated untuk menghasilkan data yang realistis
    """
    print("🔧 Generating dummy data untuk demonstrasi...")
    
    # Kota-kota di Indonesia dengan koordinat yang akurat
    cities = [
    ...
    # Jenis sarung tangan dengan variasi yang realistis
    glove_types = [
    ...
    # Brand sarung tangan dengan positioning yang berbeda
    brands = [
    ...
    for i in range(n_samples):
        # Basic info dengan variasi yang natural
        ...
        # Price dengan distribusi yang realistis (skewed distribution)
        base_price = random.randint(5000, 50000)
        price_variation = random.uniform(0.8, 1.5)
        price_number = int(base_price * price_variation)
        
        # Sold count dengan long-tail distribution
        sold_count = int(np.random.exponential(500)) + 10
        sold_count = min(sold_count, 5000)  # Cap at 5000
        
        # Rating dengan normal distribution
        rating = round(np.random.normal(4.0, 0.5), 1)
        rating = max(3.0, min(5.0, rating))  # Clamp between 3.0-5.0
        
        # Review count dengan correlation dengan sold count
        review_count = int(sold_count * random.uniform(0.1, 0.3))
        review_count = max(5, min(500, review_count))
        
        # Performance score dengan weighted formula
        ...
        # Cluster dengan distribusi yang seimbang
    ...
    print(f"✅ Generated {len(df)} samples dengan distribusi realistis")
...
# =============================================================================
# LANGKAH 2: PEMETAAN SPASIAL DENGAN FOLIUM
# =============================================================================

def create_spatial_mapping(df):
    """
    Membuat pemetaan spasial dengan Folium
    Menggunakan teknik visualisasi yang sophisticated
    """
...
def create_interactive_map(df_mapping):
    """
    Membuat peta interaktif dengan markers dan heatmap
    """
...
# =============================================================================
# LANGKAH 3: CHOROPLETH MAP SPASIAL DENGAN LEGENDA SOPHISTICATED
# =============================================================================
...
def create_choropleth_map(df_data, metric_column='Sold_Count', title="Distribusi Kasus per Wilayah"):
    ...
    Menggunakan teknik visualisasi yang advanced dengan gradasi warna yang smooth
    """
...
# =============================================================================
# LANGKAH 4: HEATMAP DENSITY DENGAN CLUSTER ANALYSIS
# =============================================================================
...
def create_density_heatmap(df_data, cluster_column='Cluster'):
    ...
    Menggunakan teknik clustering untuk analisis density yang sophisticated
    """
...
# =============================================================================
# LANGKAH 5: VISUALISASI INTERAKTIF DENGAN PLOTLY
# =============================================================================

def create_3d_scatter_plot(df):
    """
    Membuat 3D scatter plot dengan Plotly
    Menggunakan teknik visualisasi 3D yang sophisticated
    """
    ...
    if len(sorted_valid_clusters) > 0 and not df.empty:
        ...
        print("✅ 3D Scatter Plot berhasil dibuat dan disimpan.")
        return fig_3d
    else:
...
# =============================================================================
# LANGKAH 6: ANALISIS KOMPETITOR DAN REKOMENDASI STRATEGI
# =============================================================================

def analyze_competitor_positioning(df):
    """
    Analisis positioning kompetitor berdasarkan cluster
    Menggunakan teknik analisis kompetitif yang sophisticated
    """
    ...
    if not df.empty and 'Cluster' in df.columns:
        ...
        if not segment_analysis.empty:
            ...
            # Save analysis
            segment_analysis.to_excel("market_segment_analysis.xlsx")
            print("✅ Market segment analysis disimpan ke 'market_segment_analysis.xlsx'")
...
def generate_strategic_recommendations(df):
    """
    Generate rekomendasi strategi berdasarkan analisis cluster
    Menggunakan framework analisis strategi yang comprehensive
    """
...
# =============================================================================
# MAIN FUNCTION - MENJALANKAN SEMUA ANALISIS
# =============================================================================

def main():
    """
    Main function yang menjalankan seluruh analisis
    """
    print("="*70)
    print("ANALISIS KOMPETITOR DAN PEMETAAN SPASIAL DENGAN CHOROPLETH MAP")
    print("="*70)
    print("Oleh: Dosen Sains Data dengan 30 tahun pengalaman")
    print("Sertifikasi Internasional dalam Data Science dan Machine Learning")
    print("="*70)
    
    # LANGKAH 1: Generate Data
    print("\n" + "="*70)
    print("LANGKAH 1: GENERATOR DATA DUMMY")
    print("="*70)
    df = generate_dummy_data(1000)
    
    # LANGKAH 2: Spatial Mapping
    print("\n" + "="*70)
    print("LANGKAH 2: PEMETAAN SPASIAL")
    print("="*70)
    df_mapping = create_spatial_mapping(df)
    map_filename = create_interactive_map(df_mapping)
    
    # LANGKAH 3: Choropleth Maps
    print("\n" + "="*70)
    print("LANGKAH 3: CHOROPLETH MAP SPASIAL")
    print("="*70)
    ...
    if not df_mapping.empty:
        ...
        _, filename_sold = create_choropleth_map(
            df_mapping, 
            'Sold_Count', 
            "Distribusi Jumlah Penjualan per Kota"
        )
        if filename_sold:
            choropleth_files.append(filename_sold)
        
        # 2. Choropleth untuk rata-rata harga
        _, filename_price = create_choropleth_map(
            df_mapping, 
            'Price_Number', 
            "Distribusi Rata-rata Harga per Kota"
        )
        if filename_price:
            choropleth_files.append(filename_price)
        
        # 3. Choropleth untuk performance score
        if 'Performance_Score' in df_mapping.columns:
            _, filename_perf = create_choropleth_map(
                df_mapping, 
                'Performance_Score', 
                "Distribusi Performance Score per Kota"
            )
            if filename_perf:
                choropleth_files.append(filename_perf)
    
    # LANGKAH 4: Density Heatmap
    print("\n" + "="*70)
    print("LANGKAH 4: HEATMAP DENSITY")
    print("="*70)
    
    if not df_mapping.empty and 'Cluster' in df_mapping.columns:
        density_map, density_filename = create_density_heatmap(df_mapping)
    
    # LANGKAH 5: Interactive Visualizations
    print("\n" + "="*70)
    print("LANGKAH 5: VISUALISASI INTERAKTIF")
    print("="*70)
    
    fig_3d = create_3d_scatter_plot(df)
    
    # LANGKAH 6: Competitor Analysis
    print("\n" + "="*70)
    print("LANGKAH 6: ANALISIS KOMPETITOR")
    print("="*70)
    
    segment_analysis = analyze_competitor_positioning(df)
    generate_strategic_recommendations(df)
    
    # LANGKAH 7: Save Final Dataset
    print("\n" + "="*70)
    print("LANGKAH 7: MENYIMPAN HASIL ANALISIS")
    print("="*70)
    
    output_filename = "Tokopedia_sarung_tangan_with_clusters.xlsx"
    df.to_excel(output_filename, index=False)
    print(f"💾 Dataset akhir dengan kolom baru telah disimpan ke '{output_filename}'")
    ...
    if 'choropleth_files' in locals():
        for filename in choropleth_files:
            print(f"   • Choropleth map: {filename}")
    if 'density_filename' in locals():
        print(f"   • Density heatmap: {density_filename}")
    print(f"   • 3D Scatter Plot: 3d_scatter_plot.html")
    print(f"   • Market Analysis: market_segment_analysis.xlsx")
    print(f"   • Dataset lengkap: {output_filename}")
    ...
    print("   ✅ Gradasi warna dari lightgreen → yellow → orange → red")
    print("   ✅ Legend yang sophisticated dengan informasi detail")
    print("   ✅ Popup interaktif dengan statistik per wilayah")
    print("   ✅ Radius circle yang dinamis berdasarkan nilai metrik")
    print("   ✅ Sumber data: Tokopedia Digital Service")
    print("   ✅ Responsive design yang modern")
    print("   ✅ Hover effects dan animasi yang smooth")
