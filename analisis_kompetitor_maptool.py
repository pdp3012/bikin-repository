...
# --- 6. PEMETAAN SPASIAL DENGAN PLOTLY ---
print("\n" + "="*70)
...
# b. Buat peta interaktif dengan Plotly
...
if not df_mapping.empty:
    ...
    colors = ['red', 'blue', 'green', 'purple', 'orange', 'darkred', 'lightcoral', 'beige',
              'darkblue', 'darkgreen', 'cadetblue', 'mediumpurple', 'white', 'pink', 'lightblue',
              'lightgreen', 'gray', 'black', 'lightgray']

    # Buat scatter mapbox
    fig_map = go.Figure()

    # Group by cluster untuk plotting
    for cluster_id in sorted(df_mapping['Cluster'].unique()):
        cluster_data = df_mapping[df_mapping['Cluster'] == cluster_id]
        ...
        if not cluster_data.empty:
            ...
            if cluster_id_int == -1:
                ...
                color_idx = cluster_id_int % len(colors)
            ...
            for idx, row in cluster_data.iterrows():
                hover_text.append(
                    f"<b>{row['Product_Name'][:50]}...</b><br>" +
                    f"Shop: {row['Shop_Name']}<br>" +
                    f"City: {row['Shop_City']}<br>" +
                    f"Price: Rp{row['Price_Number']:,.0f}<br>" +
                    f"Sold: {row['Sold_Count']:,}<br>" +
                    f"Rating: {row['Rating']:.1f}<br>" +
                    f"Cluster: {cluster_id}<br>" +
                    f"Performance Score: {row['Performance_Score']:.2f}"
                )
    ...
    # Update layout untuk mapbox
    fig_map.update_layout(
        title='Peta Distribusi Harga Sarung Tangan - Tokopedia',
        mapbox=dict(
            style='open-street-map',
            center=dict(
                lat=df_mapping['Latitude'].mean(),
                lon=df_mapping['Longitude'].mean()
            ),
            zoom=5
        ),
        width=1200,
    ...
    # Save map sebagai HTML
    map_filename = "peta_distribusi_harga_sarung_tangan_plotly.html"
...
for cluster_id in sorted(df_clustered['Cluster'].unique()):
    cluster_data = df_clustered[df_clustered['Cluster'] == cluster_id]
    # Fix: Convert cluster_id to int for indexing
    cluster_id_int = int(cluster_id)
    color = colors[cluster_id_int % len(colors)] if cluster_id_int != -1 else 'gray'
